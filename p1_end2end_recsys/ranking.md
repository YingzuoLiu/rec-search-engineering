...
**不是"哪个模型最好"，  
而是"这个阶段 + 这个目标 + 这个延时预算，用哪个模型最合理"。**

## 二、标准工业 Ranking Pipeline

**Recall → Rough Rank（粗排） → Fine Rank（精排） → Re-rank**
...
| **样本规模** | **几千 → 几百** |
| **延时要求** | **极高（ms 级）** |
| **常用模型** | **LR / FM / DeepFM / 简化 MLP** |
| **Loss** | **Pointwise（CTR / CE）** |
| **为什么** | **稳定、可并行、易上线** |

**📌 粗排更关注稳定的相关性打分和延时控制，pointwise loss 是工程上的最优折中。**
...
| **样本规模** | **几百 → 几十** |
| **延时要求** | **可接受稍高** |
| **常用模型** | **DIN / DIEN / Transformer / MMoE / PLE** |
| **Loss** | **Pairwise / Hybrid** |
| **为什么** | **直接优化相对顺序，更贴近 Ranking 本质** |

**📌 精排阶段更关注相对顺序，因此更倾向使用 pairwise loss 或复杂行为建模模型。**
...
| **是否改模型** | **❌ 不改** |
| **常见约束** | **新商家、GMV、多样性、成本** |
| **常见方式** | **规则 / quota / heuristic** |

**📌 业务约束通常在 rerank 阶段处理，避免污染 ranking loss。**

## 四、Loss 选择地图（面试高频）

| **Loss 类型** | **用在哪** | **特点** |
|---------------|------------|----------|
| **Pointwise** | **粗排** | **稳定、快、好训练** |
| **Pairwise** | **精排** | **直接优化顺序** |
| **Listwise** | **树模型** | **效果好但成本高** |
